...
API
    ...
    oc new-app openshift/metrics-deployer-template -p CASSANDRA_PV_SIZE=9Gi -p HAWKULAR_METRICS_HOSTNAME=metrics.apps-${GUID}.oslab.opentlc.com -p USE_PERSISTENT_STORAGE=true -p IMAGE_VERSION=3.3.0 -p IMAGE_PREFIX=registry.access.redhat.com/openshift3/   2>&1 | tee -a $LOGFILE
    ansible masters -m shell -a "sed -i '/publicURL:/a \ \ metricsPublicURL: https://metrics.apps-'${GUID}'.oslab.opentlc.com'  /etc/origin/master/master-config.yaml"   2>&1 | tee -a $LOGFILE
